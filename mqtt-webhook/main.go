...
func main() {
	...
	if MQTTBroker == "" || MQTTPort == "" || MQTTUsername == "" ||
		MQTTPassword == "" || MQTTTopicSub == "" || WebhookURL == "" {
	...
	go func() {
		log.Printf("Dummy server listening on port %s\n", Port)
...
func mqttMessageHandler(webhookURL string) mqtt.MessageHandler {
	return func(client mqtt.Client, msg mqtt.Message) {
		...
		go func() {
			...
			resp.Body.Close()
			log.Printf("Forwarded message from topic %s\n", msg.Topic())
		}()
	}
}
